...
class GameViewer:
    ...
    def show_moves(self, top_n=5):
        ...
        print(
            f"  sims={sims}  time={t:.2f}s  avg_depth={avg_d:.2f}  max_depth={max_d}",
            f"children visited={node['children_visited']}/{node['total_children']}"
        )
        ...
        for c in cands[:top_n]:
            ...
            print(f"   {san:<6} visits={c.get('visits',0):<5} "
                  ...
                  f"U={c.get('U',0):+.3f}")
        print(f"\nvisit-weighted Q={node['visit_weighted_Q']}")
        print("="*60)
...
view = [g for g in all_games if (g['plies'] > 20) and (g['stockfish_color'] is not None)]
gv = GameViewer(all_games[-1]['json_file']); gv.replay()
